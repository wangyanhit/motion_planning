...
class BrrtStar:
    def __init__(self, start, goal, name):
        ...
        #pygame.display.set_caption('B-RRTstar')
    ...
    def run_brrt_star(self):
        ...
        for i in range(NUMNODES):
            ...
            self.start_time += time.time() - t1
        print 'time: {}s, len: {}'.format(time.time() - self.start_time, self.cost(sigma_new))
        pygame.image.save(self.screen, 'B-RRT*.png')
    ...
    def run_rrt_star(self):
        ...
        for i in range(NUMNODES):
            ...
            x_nearest = self.nearestVertex(x_rand, self.Ta)
            x_new = self.extend(x_nearest, x_rand)
            if self.obstacleFree((x_nearest, x_new)):
                X_near = self.nearVertices(x_new, self.Ta)
                self.nodes.append(x_new)
                self.Ta.append(x_new)
                x_min = x_nearest
                c_min = x_nearest.cost + self.dist(x_nearest, x_new)
                for x in X_near:
                    dist = x.cost + self.dist(x, x_new)
                    if dist < c_min and self.obstacleFree((x, x_new)):
                        x_min = x
                        c_min = dist
                x_new.cost = c_min
                x_new.parent = x_min
                self.edges.append((x_min, x_new))
                pygame.draw.line(self.screen, self.black, [x_min.x, x_min.y], [x_new.x, x_new.y])
                for x in X_near:
                    dist = x_new.cost + self.dist(x_new, x)
                    if dist < x.cost and self.obstacleFree((x_new, x)):
                        self.edges.remove((x.parent, x))
                        pygame.draw.line(self.screen, self.white, [x.x, x.y], [x.parent.x, x.parent.y])
                        x.parent = x_new
                        x.cost = dist
                        self.edges.append((x.parent, x))
                        pygame.draw.line(self.screen, self.black, [x.x, x.y], [x_new.x, x_new.y])
                t1 = time.time()
                pygame.display.update()
                self.start_time += time.time() - t1
        ...
        dist = 100000000
        for x in self.Ta:
            current_dist =self.dist(x, self.goal)
            if current_dist < dist:
                if self.obstacleFree((x, self.goal)):
                    x_nearest = x
                    dist = current_dist
        p = [self.goal, x_nearest]
        while x_nearest.parent != None:
            p.append(x_nearest.parent)
            x_nearest = x_nearest.parent
        print 'time: {}s, len: {}'.format(time.time() - self.start_time, self.cost(p))
        drawSolutionPath(p, pygame, self.screen)
        pygame.image.save(self.screen, 'RRT*.png')
    ...
    def run_rrt(self):
        ...
        for i in range(NUMNODES):
            ...
            if self.obstacleFree((x_nearest, x_new)):
                self.nodes.append(x_new)
                self.Ta.append(x_new)
                x_new.cost = x_nearest.cost + self.dist(x_nearest, x_new)
                x_new.parent = x_nearest
                self.edges.append((x_nearest, x_new))
                pygame.draw.line(self.screen, self.black, [x_nearest.x, x_nearest.y], [x_new.x, x_new.y])
                t1 = time.time()
                pygame.display.update()
                self.start_time += time.time() - t1
...
def main(name):
    ...
    brrt_star = BrrtStar(start, goal, name)

    if name == 'B-RRT*':
        brrt_star.run_brrt_star()
    elif name == 'RRT*':
        brrt_star.run_rrt_star()
    elif name == 'RRT':
        brrt_star.run_rrt()
...
if __name__ == '__main__':
    assert len(sys.argv) == 2, "Please choose the algorithm you want to run in following format: python brrtstar.py 'RRT'/'RRT*'/'B-RRT*'"
    algorithm = str(sys.argv[1])
    if algorithm != 'RRT' and algorithm != 'RRT*' and algorithm != 'B-RRT*':
        assert False, "Incorrect algorithm provided"
    main(algorithm)